...
func TestCanGet8BitValue(t *testing.T) {
	v := game1.GetValue(storage.Attribute{0x203, 8})
	if v != 70 {
...
func TestCanGet16BitValue(t *testing.T) {
	v := game2.GetValue(storage.Attribute{0x1FF0, 16})
	if v != 25819 {
...
func TestCanSet8BitValue(t *testing.T) {
	game1.SetValue(storage.Attribute{0x203, 8}, 255)
	v := game1.GetValue(storage.Attribute{0x203, 8})
	if v != 255 {
...
func TestCanSet16BitValue(t *testing.T) {
	game2.SetValue(storage.Attribute{0x203, 16}, 2003)
	v := game2.GetValue(storage.Attribute{0x203, 16})
	if v != 2003 {
...
func TestCanGenerateCheckSum(t *testing.T) {
	...
	g1.Save()
	c := g1.GetValue(storage.Attribute{0x1FF0, 16})
	if c != 46426 {
	...
	g2.Save()
	c = g2.GetValue(storage.Attribute{0x1FF0, 16})
	if c != 25819 {
		t.Errorf("Checksum invalid: expected %d, actual %d", 25819, c)
	}
	c = g2.GetValue(storage.Attribute{0x1FF2, 16})
	if c != 25060 {
		t.Errorf("Checksum invalid: expected %d, actual %d", 25060, c)
	}
	c = g2.GetValue(storage.Attribute{0x1FF4, 16})
	if c != 16873 {
